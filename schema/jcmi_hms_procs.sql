...
if (_workorder_completeDate IS NOT NULL) THEN set _workorder_completeDate = cast(STR_TO_DATE(_workorder_completeDate, _dateFormat) as date); END IF;
if (_workorder_rescheduleDate IS NOT NULL) THEN set _workorder_rescheduleDate = cast(STR_TO_DATE(_workorder_rescheduleDate, _dateFormat)as date); END IF;

    if (_workorder_scheduleDate IS NOT NULL) THEN
        set _workorder_scheduleDate = cast(STR_TO_DATE(_workorder_scheduleDate, _dateFormat) as date);
    ELSE
        set _workorder_scheduleDate=DATE(now());
    END IF;

     if (_workorder_dueDate IS NOT NULL) THEN
        set _workorder_dueDate = cast(STR_TO_DATE(_workorder_dueDate, _dateFormat) as date);
    ELSE
  ...
  if (_workorder_frequency > 0) THEN
    ...
    set _workorder_scheduleDate = cast(STR_TO_DATE(_date, _dateFormat) as date);
    if (_workorderUUID is null) THEN set _workorderUUID=UUID(); END IF;
