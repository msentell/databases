...
DELIMITER $$
CREATE PROCEDURE `WORKORDER_workOrder`(IN _action VARCHAR(100),
...
BEGIN
    ...
    ELSEIF (_action = 'CREATE' and _workorderUUID is not null) THEN
        ...
        if (_workorder_rescheduleDate IS NOT NULL) THEN
            set _workorder_rescheduleDate = STR_TO_DATE(_workorder_rescheduleDate, _dateFormat);
        END IF;
        ...
        insert into workorder (workorderUUID,
                               ...
                               workorder_number, workorder_name, workorder_details, workorder_actions,
                               workorder_priority,
                               workorder_dueDate, workorder_rescheduleDate, workorder_completeDate, workorder_frequency,
                               workorder_frequencyScope,
                               workorder_createdByUUID, workorder_updatedByUUID, workorder_updatedTS,
                               workorder_createdTS)
    ...
    ELSEIF (_action = 'REMOVE' and _workorderUUID is not null) THEN
        ...
        if (_wapj_asset_partUUID IS NOT NULL) THEN
            delete
            from workorder_asset_part_join
            where wapj_asset_partUUID = _wapj_asset_partUUID
              and wapj_workorderUUID = _workorderUUID;
        ELSE
            update workorder
            set workorder_deleteTS     = now(),
                workorder_updatedTS    = now(),
                workorder_updatedByUUID=_userUUID
    ...
    ELSEIF (_action = 'ASSIGN') THEN
        ...
        update workorder
        set workorder_status='OPEN',
            workorder_completeDate =null,
    ...
    ELSEIF (_action = 'START') THEN
        ...
        select workorder_checklistUUID
        into _workorder_checklistUUID
        from workorder
        where workorderUUID = _workorderUUID;
        ...
        update workorder
        set workorder_status='IN_PROGRESS',
            workorder_completeDate =null,
            workorder_updatedTS = now(),
            workorder_updatedByUUID=_userUUID,
            workorder_checklistUUID=_checklist_historyUUID
        where workorderUUID = _workorderUUID;
    ...
    ELSEIF (_action = 'COMPLETE') THEN
        ...
        update workorder
        set workorder_status='Complete',
            workorder_completeDate = DATE(now()),
            workorder_updatedTS = now(),
            workorder_updatedByUUID=_userUUID
        where workorderUUID = _workorderUUID;
        ...
        if (_workorder_checklistUUID is not null) THEN
            call CHECKLIST_checklist(
                    ...
                    null, null, _workorder_checklistUUID, null, null, null, null, null, null, null, null, null, null,
    ...
    ELSEIF (_action = 'ADDPART' and _wapj_asset_partUUID is not null) THEN
        ...
        values (_workorderUUID, _wapj_asset_partUUID, _wapj_quantity, now());
    ...
    IF (_DEBUG = 1) THEN
        select _action,
               _workorderUUID,
               _customerId,
               _workorder_locationUUID,
               _workorder_userUUID,
               _workorder_groupUUID,
               _workorder_assetUUID,
               _workorder_checklistUUID,
               _workorder_status,
               _workorder_type,
               _workorder_number,
               _workorder_name,
               ...
               _workorder_priority,
               _workorder_dueDate,
               _workorder_rescheduleDate,
               _workorder_completeDate,
               _workorder_frequency,
               _workorder_frequencyScope,
               _userUUID;
...
DELIMITER $$
CREATE PROCEDURE `CHECKLIST_checklist`(IN _action VARCHAR(100),
                                       IN _userUUID char(36),
...
BEGIN
    ...
    IF (_action = 'GET_HISTORY' and
        (_historyUUID is not null or _checklistUUID is not null or _checklist_itemUUID is not null)) THEN


        set @l_sql = CONCAT('select c.*,i.* from checklist_history c ');
        set @l_sql = CONCAT(@l_sql,
                            'left join checklist_item_history i on (i.checklist_history_item_historyUUID = c.checklist_historyUUID) ');
        set @l_sql = CONCAT(@l_sql, ' where ');
    ...
    ELSEIF (_action = 'GET_TEMPLATE' and (_checklistUUID is not null or _checklist_itemUUID is not null)) THEN
        ...
        set @l_sql =
                CONCAT(@l_sql, 'left join checklist_item i on (i.checklist_item_checklistUUID = c.checklistUUID) ');
        set @l_sql = CONCAT(@l_sql, ' where ');
    ...
    ELSEIF (_action = 'UPDATE_HISTORY' or _action = 'FAIL_CHECKLIST_CREATEWO') THEN
        ...
        if (_checklistUUID is null or _assetUUID is null) THEN
            SIGNAL SQLSTATE '41002' SET MESSAGE_TEXT =
                    'call CHECKLIST_checklist: _checklistUUID,assetUUID,_historyUUID required';
            LEAVE CHECKLIST_checklist;
        END IF;
        ...
        IF (_action = 'FAIL_CHECKLIST_CREATEWO') THEN
            ...
            set checklist_history_resultFlag=2,
                checklist_history_updatedTS=now(),
                checklist_history_updatedByUUID=_userUUID
            where checklist_historyUUID = _historyUUID;

            update workorder
            set workorder_completeDate=Date(now()),
                workorder_updatedTS = now(),
                workorder_updatedByUUID = _userUUID,
        ...
        select asset_locationUUID, asset_name
        into _workorder_locationUUID, _assetName
        from asset
        where assetUUID = _assetUUID;

        select checklist_name, checklist_rulesJSON
        into _checklist_name,_checklist_rulesJSON
        from checklist
        where checklistUUID = _checklistUUID;
        ...
        if (_foundId is null) THEN
            ...
            insert into checklist_history (checklist_historyUUID, checklist_history_checklistUUID,
                                           checklist_history_customerUUID,
                                           checklist_history_workorderUUID, checklist_history_assetUUID,
                                           checklist_history_statusId,
                                           checklist_history_name, checklist_history_rulesJSON,
                                           checklist_history_resultFlag,
                                           checklist_history_createdByUUID, checklist_history_updatedByUUID,
                                           checklist_history_updatedTS, checklist_history_createdTS)
            Values (_historyUUID, _checklistUUID, _customerUUID, _workorderUUID, _assetUUID, 1,
                    _checklist_name, _checklist_rulesJSON,
                    0,
                    _userUUID, _userUUID, now(), now());

            select group_concat(checklist_itemUUID)
            into _ids
            from checklist_item
            where checklist_item_checklistUUID = _checklistUUID
              and checklist_item_statusId = 1
            order by checklist_item_sortOrder;

            if (_DEBUG = 1) THEN
                select 'CREATE HISTORY ',
                       _workorderUUID,
                       ' ',
                       _assetUUID,
                       ' ',
            ...
            if (_ids is not null) then
                ...
                loop
                    ...
                    select UUID(),
                           checklist_item_sortOrder,
                           checklist_item_prompt,
                           checklist_item_type,
                           checklist_item_optionSetJSON,
                           checklist_item_successPrompt,
                    ...
                    from checklist_item
                    where checklist_itemUUID = _id;


                    insert into checklist_item_history (checklist_history_itemUUID, checklist_history_item_historyUUID,
                                                        checklist_history_item_statusId,
                                                        checklist_history_item_sortOrder, checklist_history_item_prompt,
                                                        checklist_history_item_type,
                                                        checklist_history_item_optionSetJSON,
                                                        checklist_history_item_successPrompt,
                                                        checklist_history_item_successRange,
                                                        checklist_history_item_resultFlag,
                                                        checklist_history_item_createdByUUID,
                                                        checklist_history_item_updatedByUUID,
                                                        checklist_history_item_updatedTS,
                                                        checklist_history_item_createdTS)
                    values (_checklist_itemUUID, _historyUUID, 1,
...
*/
        ...
        ELSE -- history found, so update records
            ...
            if (_checklist_statusId is not null or _checklist_name is not null) THEN
                ...
                set @l_sql =
                        CONCAT('update checklist_history set checklist_updatedTS=now(), checklist_updatedByUUID=\'',
                               _userUUID, '\'');
            ...
            if (_checklist_item_statusId is not null or _checklist_name is not null) THEN
                ...
                set @l_sql = CONCAT(
                        'update checklist_item_history set checklist_history_item_updatedTS=now(), checklist_history_item_updatedByUUID=\'',
                        _userUUID, '\'');

                if (_checklist_history_item_resultText is not null) THEN
                    set @l_sql = CONCAT(@l_sql, ', checklist_history_item_resultText= \'',
                                        _checklist_history_item_resultText, '\'');
                END IF;
                if (_checklist_history_item_resultFlag is not null) THEN
                    set @l_sql =
                            CONCAT(@l_sql, ', checklist_history_item_resultFlag= ', _checklist_history_item_resultFlag);
                END IF;
    ...
    ELSEIF (_action = 'UPDATE_TEMPLATE' and _checklistUUID is not null) THEN
        ...
        if (_foundId is null) THEN
            ...
            insert into checklist (checklistUUID, checklist_customerUUID, checklist_statusId, checklist_name,
                                   checklist_recommendedFrequency,
                                   checklist_rulesJSON,
                                   checklist_createdByUUID, checklist_updatedByUUID, checklist_updatedTS,
                                   checklist_createdTS)
            values (_checklistUUID, _customerUUID, 1, _checklist_name, _checklist_recommendedFrequency,
        ...
        ELSE
            ...
            if (_checklist_recommendedFrequency is not null) THEN
                set @l_sql =
                        CONCAT(@l_sql, ',checklist_recommendedFrequency = \'', _checklist_recommendedFrequency, '\'');
            END IF;
            if (_checklist_name is not null) THEN
        ...
        if (_foundId is null and _checklist_itemUUID is not null) THEN
            ...
            insert into checklist_item (checklist_itemUUID, checklist_item_checklistUUID, checklist_item_customerUUID,
                                        checklist_item_statusId,
                                        checklist_item_sortOrder, checklist_item_prompt, checklist_item_type,
                                        checklist_item_optionSetJSON,
                                        checklist_item_successPrompt, checklist_item_successRange,
                                        checklist_item_createdByUUID, checklist_item_updatedByUUID,
                                        checklist_item_updatedTS, checklist_item_createdTS)
            values (_checklist_itemUUID, _checklistUUID, _customerUUID, 1,
                    _checklist_item_sortOrder, _checklist_item_prompt, _checklist_item_type,
                    _checklist_item_optionSetJSON,
                    _checklist_item_successPrompt, _checklist_item_successRange,
                    _userUUID, _userUUID, now(), now());
        ...
        ELSE
            ...
            set @l_sql =
                    CONCAT('update checklist_item set checklist_item_updatedTS=now(), checklist_item_updatedByUUID=\'',
                           _userUUID, '\'');
    ...
    ELSEIF (_action = 'PASS_CHECKLIST') THEN
        ...
        select checklist_history_workorderUUID
        into _workorderUUID
        from checklist_history
        where checklist_historyUUID = _historyUUID;

        update checklist_history
        set checklist_history_resultFlag=1,
            checklist_history_updatedTS=now(),
            checklist_history_updatedByUUID=_userUUID
        where checklist_historyUUID = _historyUUID;
